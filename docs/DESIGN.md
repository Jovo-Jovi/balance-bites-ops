...
hub/src/lib/design/
  ...
  icon-catalog.json 260 icons (Kids includes pizza/chicken/ketchup/toys/pebble/BB bowl)
  art.ts            bg slots, stamps, addProductPhotos, fill-cut-with-paper
  art-presets.ts    artref: / assets/presets/ → /design-presets/*.svg; compact pack keys on Brand rail only
...
hub/public/design-presets/              repo character SVGs (not Firestore). Compact pack: `bb-lockup`, `bb-sudani-beans`, `bb-stone-chocolate`, `bb-surprise-toys` (original kawaii, not vtracer dumps)
```
