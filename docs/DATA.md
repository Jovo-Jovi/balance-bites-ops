...
Staff can download a local zip of **existing** Firestore keys from the hub footer, any workspace footer, or Finance → التشغيل → نسخ احتياطية (`bb-saved-data-YYYY-MM-DD.zip`, `{key}.json` files). Missing keys are skipped (no empty dumps). Label art on R2 is not in the zip.
