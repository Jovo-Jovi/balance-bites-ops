...
9. Finished goods: **فواتير** (`soldGross`) = approved invoice qty. **مباع** (`sold`) = after invoice-linked returns. **رصيد** is the typed shelf count. Editing it adjusts **that product’s** إنتاج only (إنتاج = فواتير − restock + hawalek + رصيد) and that recipe’s ingredients. Other SKUs stay put. Production **ينقص** = max(0, فواتير − produced). Prep board / drafts are not in these columns. Prep **إلى اللوحة** with a customer also writes that customer’s invoice draft and stores their name on the board line.
