...
**Shipped:** hub at `hub/` (Vercel root `hub`). Invoices native React on `main`. Design Waves A–B on `main` ([DESIGN.md](DESIGN.md)). Studio Wave C in test: [DESIGN-STUDIO.md](DESIGN-STUDIO.md). Finance is still a shell. HTML wrap into `public/` was **not** used.
